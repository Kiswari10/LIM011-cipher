window.cipher = {
  encode: (offset, string) => {
    ...
    for (let count = 0; count < string.length; count += 1) {
      ...
      if (element >= 65 && element <= 90) { // primera condicional:mayúsculas
        const change = ((element - 65 + offset) % 26) + 65;
        letter = String.fromCharCode(change);
      } else if (element >= 97 && element <= 122) { // segunda condicional:minúsculas
        const change = ((element - 97 + offset) % 26) + 97;
        letter = String.fromCharCode(change);
      } else {
        letter = string[count]; // otros caracteres
      }
      newMessage += letter; // concatenación de strings
    } return newMessage;
  },
